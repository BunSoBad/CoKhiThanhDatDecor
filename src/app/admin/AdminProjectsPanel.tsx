...
import type { ProjectAdminRow } from "@/lib/data";

export function AdminProjectsPanel({
  initialProjects,
}: {
  initialProjects: ProjectAdminRow[];
}) {
  ...
  function loadProject(p: ProjectAdminRow) {
    setEditingId(p.id);
    setNameVi(p.nameVi);
    setSlug(p.slug);
    ...
    <section className="admin-card rounded-2xl border border-slate-200/90 bg-white shadow-sm">
      <div className="border-b border-slate-200 px-5 py-4">
        <h2 className="text-base font-semibold text-slate-950">
          Quản lý dự án (trang chủ: dự án tiêu biểu)
        </h2>
        <p className="mt-1 text-sm text-slate-500">
          Nhập thông tin và ảnh bìa, gallery — lưu vào cơ sở dữ liệu. Bật
          &quot;Hiển thị tiêu biểu&quot; để hiện ở khối dự án tiêu biểu trên
          trang chủ.
        </p>
      </div>
      ...
      >
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <label className="block sm:col-span-2">
            <span className="text-xs font-semibold text-slate-600">
              Tên dự án / case <span className="text-rose-600">*</span>
            </span>
            <input
              required
              value={nameVi}
              ...
              placeholder="Ví dụ: Dự án cổng trượt tự động"
            />
          </label>
          ...
          <label className="flex items-center gap-2 pt-6">
            ...
            <span className="text-sm font-medium text-slate-800">
              Hiển thị ở mục dự án tiêu biểu (trang chủ)
            </span>
          </label>
        </div>

        <label className="block">
          <span className="text-xs font-semibold text-slate-600">Tóm tắt</span>
          <textarea
            value={summary}
        ...
        <div>
          ...
          <div className="mt-1 flex flex-wrap items-center gap-2">
            ...
            <input
              ...
              className="min-w-[12rem] flex-1 rounded-lg border border-slate-200 px-3 py-2 font-mono text-xs"
        ...
        <div className="flex flex-wrap gap-2">
          <button
            type="submit"
            disabled={loading}
            className="rounded-lg bg-emerald-600 px-4 py-2 text-sm font-bold text-white shadow hover:bg-emerald-700 disabled:opacity-50"
          >
        ...
        <table className="min-w-full divide-y divide-slate-200 text-left">
          <thead className="bg-slate-50/90 text-[11px] uppercase tracking-wider text-slate-500">
            <tr>
              ...
              <th className="px-4 py-3 font-semibold">Tiêu biểu</th>
            ...
            {initialProjects.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-4 py-8 text-sm text-slate-500">
                  Chưa có dự án trong cơ sở dữ liệu. Thêm mới ở form phía trên.
                </td>
              </tr>
            ) : (
